...
* *assignments migrate-from-program* – выполняет миграцию выполненных домашних заданий

    Эта команда выполняется для того, чтобы сохранить решение старых домашних заданий. Команда выполняет следующие действия:

    * Скачивает выполненные домашние задания с Gitlab
    * Добавляет их в директорию *Hexlet/hexlet-assignments* на компьютере студента  и в репозитиорий *hexlet-assignments* в профиле студента на Github
    * Закрывает пуш в старый репозиторий студента на GitLab
...
Команда *assignments migrate-from-program* использует аргумент *program*. Это название учебной программы, для которой нужно выполнить миграцию домашних заданий
...
Для сохранения решения имеющихся домашних заданий нужно будет выполнить миграцию. Миграцию нужно выполнить один раз и только в том случае, если вы начали выполнять домашние задания по старой схеме и у вас уже имеются выполненные домашние задания на GitLab. Она выполняется при помощи команды *hexlet assignments migrate-from-program*. В качестве аргумента команде нужно передать название программы, для которой выполняется миграция:

```bash
# Можно вызывать из любого каталога
# В качестве параметра передаем название учебной программы

hexlet assignments migrate-from-program java
```

Команда миграции закроет пуш в репозиторий на GitLab, дальнейшая работа с домашними заданиями будет вестись в GitHub по новой схеме. Поэтому перед миграцией нужно завершить работу по всем открытым Issue
